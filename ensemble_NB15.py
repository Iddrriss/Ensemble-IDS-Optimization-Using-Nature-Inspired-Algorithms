...
# ── Load UNSW-NB15 preprocessed data ─────────────────────────────────────────
print("Loading UNSW-NB15 data...")
X = np.load(os.path.join(BASE_PATH, "NB15_X.npy"))
y = np.load(os.path.join(BASE_PATH, "NB15_y.npy"))

with open(os.path.join(BASE_PATH, "NB15_feature_names.txt")) as f:
    feature_names = f.read().splitlines()

N_FEATURES = X.shape[1]  # 42
print(f"Features: {N_FEATURES}, Samples: {X.shape[0]}")

# ── Sample for speed during feature selection ─────────────────────────────────
SAMPLE = 5000
...
def run_pso(n_particles=20, iterations=30):
    ...
    print(f"[PSO] Done — {int(mask.sum())} features selected, fitness={gbest_score:.4f}")
    return mask
...
def run_ga(pop_size=20, generations=30, crossover_rate=0.8, mutation_rate=0.02):
    ...
    print(f"[GA] Done — {int(best_ind.sum())} features selected, fitness={best_score:.4f}")
    return best_ind
...
def run_gwo(n_wolves=20, iterations=30):
    ...
    print(f"[GWO] Done — {int(mask.sum())} features selected, fitness={alpha_score:.4f}")
    return mask
...
votes         = pso_mask + ga_mask + gwo_mask
ensemble_mask = (votes >= 2).astype(int)
selected      = np.where(ensemble_mask)[0]
selected_names = [feature_names[i] for i in selected]

print("\n" + "="*60)
print("ENSEMBLE RESULT — UNSW-NB15")
print("="*60)
...
print(f"Ensemble selected: {len(selected)} features (majority vote)")
print(f"\nSelected feature names:")
for i, name in enumerate(selected_names, 1):
    print(f"  {i:2}. {name}")

# ── Save ──────────────────────────────────────────────────────────────────────
np.save(os.path.join(BASE_PATH, "NB15_ensemble_mask.npy"),  ensemble_mask)
np.save(os.path.join(BASE_PATH, "NB15_selected_features.npy"), selected)

with open(os.path.join(BASE_PATH, "NB15_selected_feature_names.txt"), "w") as f:
    f.write("\n".join(selected_names))

print("\nAll masks saved. Ready for Step 7 classifiers.")